...
| `display.ts` → `axisLabels` | One `moneyAxisLabels(obfuscated, …)` on the shared display edge (dimensions from `CHART_AXIS_FONT_SIZE` / `CHART_AXIS_WIDTH`), spread over the money `YAxis` of **all three** charts (`PlanChart`, `CompareChart`, `CoastChart`) — hidden amounts drop the labels, so the axis keeps its scale and the plot gains the 60px gutter. `axisTick` stays the plain `formatCompactCurrency`: it never runs while the labels are gone. |
