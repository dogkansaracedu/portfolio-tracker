...
| `PlanChart.tsx` | `ComposedChart`: range `Area` (pessimistic–optimistic, `--primary` at 12%) + base `Line` (`var(--primary)`), `ReferenceLine`s for the retirement age (`insideTopRight`), the retirement target — labelled with its own formatted value — the earliest retirement age when the "when can I retire?" answer is on screen (`insideTopLeft` **plus a `dy` offset**, so the two age labels sit on opposite sides *and* different rows and cannot overprint at any width), a destructive-toned `depletionAge` marker for the pessimistic band, and — only when `contributionEndAge < retirementAge` — the coast marker pair. Points come from `buildBandPoints`, whose values are floored at 0, so the y-domain starts at zero. `PlanTab` asks `projectScenario` for `includeRetirementDrawdown` under **both** withdrawal strategies, so the line always carries on past retirement to `depletionAge` — down to zero when depleting, typically still rising under preservation. |
