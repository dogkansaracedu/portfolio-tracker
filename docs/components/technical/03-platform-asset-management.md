...
### Assets
- `src/components/assets/AssetList.tsx` — splits assets into `ownedAssets` / `otherAssets` by net balance (sums `useHoldings()` balances per `asset_id` via `bn`/`BN_ZERO`, owned = `net.gt(0)`); renders owned first, the rest behind a collapsible "Not held (N)" row. Owns the `AssetForm` and the deactivate `AlertDialog`. Header shows `is_active` count / total. Gates write controls on `useIsAdmin()`: hides "Add Asset" and passes `canManage={isAdmin}` to each `AssetRow`.
- `src/components/assets/AssetRow.tsx` — one table row: `AssetIcon` + ticker/name, category `Badge` (via `CATEGORY_LABELS`), price cell, tags as outline badges, status badge, actions menu. Price = `prices[asset.price_id ?? asset.ticker]`; native currency from `assetNativeCurrency(asset)`; fiat shows `1`; non-USD shows native value + `~$` from `price_usd`. Takes a `canManage` prop; renders its actions menu only when `canManage && is_active && !is_currency`.
- Both `AssetList` and `AssetRow` gate the write surface on `useIsAdmin()` (`src/hooks/useIsAdmin.ts`, compares `useAuth().user?.id` to `ADMIN_USER_ID` in `src/lib/constants/admin.ts`) — the UI mirror of the database's admin-write boundary.
- `src/components/assets/AssetForm.tsx` — `Dialog` with category/price-source `Select`s, ticker, display name, icon URL (+ live `AssetIcon` preview), price id, tags `Input`. Category options come from `ASSET_CATEGORIES` (`src/lib/constants/assets.ts`, includes `fund`); local `PRICE_SOURCES` / `TICKER_HINTS` constants for the rest. When `category === "fund"` it also renders an `at_source_tax_rate` number `Input` (fraction 0–1, e.g. `0.175`). On submit: requires ticker + name; validates the rate is a 0–1 fraction when non-blank; `price_id` empty ⇒ `trimmedTicker`; `icon_url` empty ⇒ `null`; `at_source_tax_rate` blank ⇒ `null` else `Number(...)`; tags split on comma → trim → lowercase → filter; always submits `is_active: true`. Create passes `is_currency: false`. The rate is threaded through `AssetInsert` / `AssetUpdate` (`src/types/database`) into the `createAsset` / `updateAsset` queries.
...
- `src/lib/constants/assets.ts` — `ASSET_CATEGORIES` (`fiat` / `crypto` / `gold` / `fund` / `stock_us` / `stock_bist`; `fund` is a TRY money-market fund / PPF, default `manual` price source), `PRICE_SOURCES`, `TICKER_HINTS`, `DEFAULT_PRICE_SOURCE` per category, `PHYSICAL_GOLD_TICKER` (`XAU_GRAM`), and `assetNativeCurrency()` / `currencyForAssetId()` (the native-currency rule — `fund` resolves to `TRY`).
- `src/lib/constants/brokers.ts` — `MIDAS_PLATFORM_NAME` (referenced by import-specific logic elsewhere).

## Data layer
Only Component-2 tables. The one schema addition this surface writes is the nullable `at_source_tax_rate numeric` column on `assets` (migration `supabase/migrations/20260608000000_asset_at_source_tax_rate.sql`) — a fraction like `0.175` withheld at source on the asset's gains; `null` for assets with no at-source tax. The P&L engine consumes it (see Component 6); this surface only edits it.
...
- `src/lib/queries/assets.ts` — `fetchAssets` (`select *`, `order name`, **no `user_id` filter** — the catalog is global), `createAsset`, `updateAsset`, `deactivateAsset` (sets `is_active = false`). Also hosts `resolveTickers()` (calls the `resolve-tickers` Edge Function) used by import elsewhere — not part of this CRUD surface. `fetchAssets()` has a second call site: `src/components/transactions/sheet/autoResolveSentinels.ts` (duplicate-ticker race recovery).
- `src/contexts/PlatformsContext.tsx` / `src/contexts/AssetsContext.tsx` — providers holding the single shared fetch + optimistic-ish mutators (`add/edit/remove` for platforms; `add/edit/deactivate` for assets, which `refetch()` after each write). `AssetsContext.refetch` calls `fetchAssets()` with no argument. Exposed via `usePlatformsContext` / `useAssetsContext`.
...
## Notes & gotchas
- **Assets are a global catalog, not per-user.** RLS on `assets` is `SELECT USING (true)` for all authenticated + INSERT/UPDATE/DELETE gated to the hardcoded admin uuid (migration `20260610000000_global_asset_catalog.sql`). `ADMIN_USER_ID` (`src/lib/constants/admin.ts`) mirrors that uuid for the UI gate via `useIsAdmin()` — RLS is the real boundary, the UI gate is convenience. Platforms remain per-user. (Note: implicit asset creation during transaction import — `autoResolveSentinels` → `createAsset` — therefore only succeeds for the admin.)
- **Shared fetch, not per-call-site:** assets and platforms are fetched once in the providers and shared. Fetching on mount inside `useAssets`/`usePlatforms` instead would fire one identical `?select=*` request per call site (`AppLayout`, filters, bulk sheet, settings) on every page. Do not reintroduce per-component fetching; consume the context.
...
- Shipped `PRICE_SOURCES` are `yahoo` / `tcmb` / `tefas` / `manual`. The `coingecko` value named in the [glossary](../GLOSSARY.md#asset) is not among them — crypto prices via `yahoo`.
