...
| `src/components/vehicle/CostEntryForm.tsx` | Add/edit a cost entry, including the fuel-only fields and the item checkboxes that reset intervals. Takes `prefillItemIds` (which items a visit closes) **and** `prefillValues` (category/amount/litres/note), the latter so the monthly fuel estimate can seed a real row. |
