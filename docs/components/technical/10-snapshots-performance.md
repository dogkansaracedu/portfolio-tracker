...
| `src/components/performance/PerformanceSummary.tsx` | Stat-card grid: current value, All-Time Return (MWR — the `allTimeMwrPct` prop, lifetime cumulative `computeLifetimeMwrCumulativePct` from `usePerformance`), CAGR, best/worst month, max drawdown. Range-scoped cards suffixed with the active range. |
...
| `src/hooks/useBenchmark.ts` | Lazy-loads one benchmark ticker's daily-close series; empty until a ticker is passed. **Consumed by the dashboard hero overlay**, not `PerformancePage` — that is where the benchmark comparison surfaces. |

### Lib

| File | Role |
| --- | --- |
| `src/lib/performance.ts` | The compute core. `TimeRange` union (`1D 1W 1M 3M 6M YTD 1Y 2Y ALL` — selector currently exposes a subset), `filterByTimeRange` (with pre-cutoff start-anchor for ≥1M ranges), `subPeriodReturn` (the per-snapshot **money-weighted (XIRR) period return** — the shared building block for monthly returns and TWR's geometric linking; returns `{ returnFraction, returnUsd, hadExternalFlow, spanDays }`), `computeMonthlyReturns` (calls `subPeriodReturn`, labels each as a monthly return), **`computeTWRSeries`** (the portfolio **time-weighted return** — geometrically chains `subPeriodReturn` across the window, which is what cancels each period's external cash flow out of the product; rebased to 0% at the first snapshot; value-weighting is automatic because each period reads the snapshot **total**; returns `TWRSeries = { points: { date, cumulativePct }[], endPct, approximate }`, where `approximate` flips true once a period that contained a flow spanned > 1 day, and an unsolvable period contributes a neutral factor), `computeCAGR`, `computeDrawdown`, `computeCategoryAttribution`, `computePerformanceMetrics`, **`computePnLTimeSeries`** (per-snapshot `value − cumulative net invested`), `computeCurrentInvestedUsd`, and `applyTxToInvested` (the canonical per-tx invested-capital rule). Worked TWR cases (incl. the +8% chain, the +35.7% withdrawal case and the +69.72% mid-period-flow XIRR case) live in `src/lib/twr.test.ts` / [pnl-test-cases.md](../../pnl-test-cases.md). Also **exports `externalCashFlowUsd`, `collectPairedParentIds` and `sortSnapshotsAsc`** — not for the UI, but so `lib/mwr.ts` classifies flows and orders windows identically instead of re-deriving them. |
| `src/lib/xirr.ts` | The **XIRR solver leaf** both return engines depend on — `solveXirrLog1p` / `deannualizeLog1p` / `solveXirr`, plus `yearsBetween`, `MS_PER_DAY`, `DAYS_PER_YEAR` and the `XirrFlow` type. Imports nothing from `performance.ts` or `mwr.ts` (that is the point: `subPeriodReturn` consumes the solver while `mwr.ts` consumes `performance.ts`'s flow helpers, so the solver has to sit below both). Full contract, bracket rationale and the log-space precision trap in [07-dashboard.md](07-dashboard.md#mwr--xirr-measure-implementation-contract). |
| `src/lib/mwr.ts` | The **money-weighted (MWR / XIRR)** engine — TWR's companion: same inputs, opposite question (TWR scores the strategy, MWR scores the investor's dollars, so deposit size/timing count). `solveXirr(flows, terminalValueUsd, terminalDate)` finds the annual rate by **bracketed bisection on `s = ln(1+r)`** over `[ln(0.0001), 400]` (a linear bracket can't hold short windows — a +50% day annualizes to `r ≈ 2e64`), returning `null` on degenerate input / no sign change rather than a fabricated 0. **`computeMWRSeries`** mirrors `computeTWRSeries`' call shape and window semantics: point *i* is the XIRR of [window start → snapshot *i*] with the start `total_usd` as an opening inflow and the same `(periodStart, periodEnd]` flow boundary as `subPeriodReturn`, reported as the de-annualized cumulative `(1+r)^years − 1` rebased to 0% at the first point (a point with no solution carries the previous one); returns `MWRSeries = { points: { date, cumulativePct }[], endPct, annualizedEndPct }`, `annualizedEndPct` null under a 1-year span. Needing only endpoint values + dated flows, it is exact even on weekly-sampled history — hence **no `approximate` flag**. `computeLifetimeXirrPct` is the annualized "%/yr" chip (all flows, `V_start = 0`, ≥ 1 year of history); `computeWhatIfIndexMWRSeries` replays the same flows into a benchmark's closes and measures the synthetic series the same way (the [what-if index](../GLOSSARY.md#what-if-index-same-flows-benchmark)); `closesAtOrBefore` is the shared close lookup. ACT/365.25 throughout. Worked cases (Excel-parity XIRR, the MWR +24.6% vs TWR +60.5% divergence, deposit-timing sensitivity, what-if index) live in `src/lib/mwr.test.ts`. Consumed by the dashboard hero's MWR measure (component 7). |
...
| `src/lib/constants/benchmarks.ts` | `BENCHMARKS` (`SPY`, `QQQ`), `DEFAULT_BENCHMARK_ID = SPY`, `findBenchmark`, and the `BENCHMARK_NONE` (`"none"`) sentinel, which `findBenchmark` resolves back to the first benchmark. Tickers must match what `fetch-benchmark-history` upserts. |
...
All three are configured in `supabase/config.toml` (`[functions.take-snapshots]`, `[functions.backfill-snapshots]`, `[functions.take-intraday-snapshots]`, alongside `[functions.fetch-prices]`).
...
- It does **not** call `take-snapshots` directly. It POSTs `fetch-prices` with body `{"force": true, "snapshot": true}` — force refetches all prices/rates, and `snapshot=true` **chains `take-snapshots`** once prices are fresh. This is the "chain after the price fetch" guarantee.
...
- `fetch-benchmark-history` pulls Yahoo's full **10y** range and upserts `onConflict (ticker,date)`, so a single run backfills any gap and the job is idempotent (weekend/holiday runs re-write the prior close). Without a scheduled invocation the table would freeze at whatever was last written and the hero's benchmark overlay would forward-fill a flat line.

## Notes & gotchas

- **Snapshot is the single source of truth for totals.** Don't add a parallel `holdings × prices` total path on these pages — two total paths drift apart. Cost basis / FIFO / realized P&L still come from `transactions` (deterministic), and per-row *display* value uses live `holdings.balance × snapshot price_usd` (not the snapshot's frozen `value_usd`) so a fresh transaction's quantity shows immediately. See [snapshot-price / live-quantity](../GLOSSARY.md#snapshot-price-and-live-quantity).
- **Auto-refresh-today effect** (`SnapshotsContext`): trails two signals — `lastUpdated` from prices (debounce **5s**, coalesces the cache-load→stale-refresh burst into one write) and `txVersion` from the transaction context (debounce **200ms**). Shorter window wins when both fire. Each signal is deduped via a ref so the effect can't loop on its own writes. Tx path calls `createSnapshot` (re-reads holdings post-recalc); price path builds from in-memory holdings and **skips the upsert when today's `total_usd`/`total_try` are unchanged** (BigNumber `.eq`) — without that check every price tick would write a snapshot row. After a write both paths call `load()` to re-read; **`load()` flips `loading` only on the first load per user** (`loadedForUserRef` guards it) — later reloads refresh silently behind the data on screen, because `loading` gates full-page skeletons (`DashboardPage`'s `loading || pnl.loading` branch) and re-entering it blanked the dashboard on tab refocus whenever the refocus price refresh had moved today's total.
...
- **The hourly cron (`supabase/migrations/20260615000100_schedule_intraday_snapshot_cron.sql`).** pg_cron job `hourly-intraday-snapshot`, `0 * * * *`. POSTs `fetch-prices` with `{"force": true, "intraday": true}` — force-refreshes prices, and `intraday=true` chains `take-intraday-snapshots`. Same Vault secrets as the daily job. Both chained calls go through `fetch-prices`' shared `triggerChainedFunction(fnName, cronToken)` helper.
- **Backfill density is configured in `backfill-snapshots/index.ts`** (the `"monthly"` branch). The Settings card's two options map to `granularity: "monthly" | "tx_dates"` — the value is named `"monthly"` but the UI label and behavior are "weekly + last 30 days daily". Change density constants there, not in the client.
...
- **The All-Time Return (MWR), `computeCAGR`, and `computeCategoryAttribution` anchor on transactions, not snapshots** — deliberately, so metrics don't lie when snapshot history begins after the first deposit. Only the value/drawdown/monthly-returns *series* read snapshot totals. (`computeAllTimeReturn` — pnl ÷ current invested — was removed 2026-08-28 in favour of the lifetime MWR.)
