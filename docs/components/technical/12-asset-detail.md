...
| `AssetHistoryChart.tsx` | Recharts `ComposedChart`: `Area` = value (display currency), `Line type="stepAfter"` = cost basis on the **value** axis (`var(--chart-4)`; TRY display converts `costBasisUsd × usdTry` per point — each date's frozen rate, never today's), `Line` = `priceUsd` on a right-hand axis; "Cost" and "Price" as shadcn `Toggle`s (independent on/off switches, deliberately not the segmented control beside them) each carrying a `SeriesDot` in its series' own stroke (`COST_SERIES_COLOR` / `PRICE_SERIES_COLOR`), so the toggles double as the chart's key — three series across two axes are otherwise unnamed, `TimeRangeSelector` (reused from performance, now a `SegmentedControl`) above, x-axis `minTickGap={48}`; money ticks go through `formatCompactCurrency`, and below `md` (`useMediaQuery(MEDIA_QUERY.md)` — Recharts sizes its plot in JS, so a Tailwind variant would still reserve the width) the value axis is pinned to `width={44}` and the price axis renders `tick={false} width={0} axisLine={false}`; the live "now" point is appended by `useAssetDetail` (held positions only, with `costBasisUsd` = the engine's current figure). `< 2` in-range points → "not enough history" hint. Exported via `LazyChart.tsx` + `<Suspense>` (Recharts stays code-split). |
