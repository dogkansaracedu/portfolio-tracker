...
| `src/lib/portfolio/assetHistory.ts` | Pure helpers, unit-tested: `buildAssetHistory(snapshots, ticker)` → `[{ date, valueUsd, valueTry, priceUsd, amount }]` by summing the ticker's `breakdown.by_asset` entries per snapshot (platform slices summed; `price_usd` taken from the entries); `filterHistoryByRange(points, range)` — same cutoff + pre-range-anchor semantics as `filterByTimeRange` in `lib/performance.ts`, applied to history points. Dates whose snapshot lacks the ticker produce no point. |
...
| `AssetHistoryChart.tsx` | Recharts `ComposedChart`: `Area` = value (display currency), `Line` = `priceUsd` on a right-hand axis, a "Price" toggle button for the line, `TimeRangeSelector` (reused from performance) above; the live "now" point is appended by `useAssetDetail` (held positions only). `< 2` in-range points → "not enough history" hint. Exported via `LazyChart.tsx` + `<Suspense>` (Recharts stays code-split). |
