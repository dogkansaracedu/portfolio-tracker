...
| `src/lib/pnl/assetReturns.ts` | Pure `computeAssetReturnRates(transactions, rates, currentValueUsd, todayIso)` → `{ totalPnlUsd, totalPnlPct, mwrCumulativePct, mwrAnnualizedPct }`. Folds the asset's txs through `applyTxToInvested` (exported from `lib/performance.ts` for this) to get net invested, its running **peak** (the % denominator; null when ≤ 0), and the per-tx deltas as `XirrFlow`s over the **asset boundary** — NOT `externalCashFlowUsd`, whose paired-cash netting is portfolio-boundary semantics. XIRR via `solveXirrLog1p` + `deannualizeLog1p` (`lib/xirr.ts`): cumulative at any age; annualized only when `yearsBetween(firstFlow, today) ≥ MIN_ANNUALIZATION_YEARS` (exported from `lib/mwr.ts` so the gate can't drift from the dashboard's). Unit-tested (`assetReturns.test.ts`), incl. the exact `1000x²−600x−720=0 → 20%/yr, +44% cumulative` case. |
