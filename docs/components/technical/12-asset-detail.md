...
| `AssetPositionSummary.tsx` | Stat cards, in two tiers via `Stat`'s `emphasis` prop (`text-xl` vs `text-sm`, every card `h-full` so the grid rows stay level). Promoted **and rendered first**: value, **Total P&L** (amount + `mwrCumulativePct`, then one muted line carrying the `MWR_LABEL` hint tooltip and — only when `mwrAnnualizedPct` is non-null — `≈+Y%/yr` at `DECIMALS.percentageRate`), daily return ("—" guards as on Portfolio). Supporting tier: unrealized return (net headline + `gross … · −… tax` annotation when `taxAccrualUsd > 0`, mirroring `PortfolioRow`; its `netPct` is the net-over-cost ratio, and **null** — so no percent renders — when a taxed position's `costBasisUsd <= 0`, since falling back to the gross `unrealizedPnlPct` there paired a net amount with a gross percent), realized (amount + `realizedPnlPct`), quantity, avg cost (native + USD), allocation. Money comes from `useDisplayMoney` (`money`/`signedMoney`/`display`), colours from `gainLossToneClass`. The MWR hint is the shared `HintPopover` (hover *and* tap), never a `title`. The "Today" card is `max-md:col-span-2`, so in the phone's 2-column grid the three promoted figures fill their own rows before the supporting tier. Sold-out variant: "no current position" + the Total P&L and Realized cards. |
| `AssetPlatformTable.tsx` | Per-platform breakdown from the asset's `HoldingPnL` slices, two layouts from one list: a `sm:hidden` stack (platform line, then quantity · value · return) and the `hidden sm:table` `Table` (platform dot + name, quantity, cost basis, value, unrealized return) — the money columns through `useDisplayMoney`. Hidden when no nonzero slice. |
| `AssetHistoryChart.tsx` | Recharts `ComposedChart`: `Area` = value (display currency), `Line type="stepAfter"` = cost basis on the **value** axis (`var(--chart-4)`; TRY display converts `costBasisUsd × usdTry` per point — each date's frozen rate, never today's), `Line` = `priceUsd` on a right-hand axis. Privacy: the VALUE axis takes `moneyAxisLabels(obfuscated, …)` (labels dropped while amounts are hidden — the phone plot goes 272px → 316px) and the tooltip masks Value + Cost via `formatMoney`; the unit `Price` keeps `formatCurrency` on both axis and tooltip — a public market quote, matching `AssetDetailHeader`'s price on the same screen. (The price axis already dropped its labels below `md` for width.)  "Cost" and "Price" as shadcn `Toggle`s (independent on/off switches, deliberately not the segmented control beside them) each carrying the shared `SeriesDot` (`src/components/common/SeriesDot.tsx`) in its series' own stroke (`COST_SERIES_COLOR` / `PRICE_SERIES_COLOR`), so the toggles double as the chart's key — three series across two axes are otherwise unnamed, `TimeRangeSelector` (reused from performance, now a `SegmentedControl`) above, x-axis `minTickGap={48}`; money ticks go through `formatCompactCurrency`, and below `md` (`useMediaQuery(MEDIA_QUERY.md)` — Recharts sizes its plot in JS, so a Tailwind variant would still reserve the width) the value axis is pinned to `width={44}` and the price axis renders `tick={false} width={0} axisLine={false}`; the live "now" point is appended by `useAssetDetail` (held positions only, with `costBasisUsd` = the engine's current figure). `< 2` in-range points → "not enough history" hint. Exported via `LazyChart.tsx` + `<Suspense>` (Recharts stays code-split). |
| `AssetIncomeCosts.tsx` | Cards for income / taxes / fees (display currency via `useDisplayMoney`, obfuscation-aware); zero-valued cards omitted. |
