...
| `src/lib/prices.ts` | Formatters + staleness: `formatCurrency` (symbol prefixed, minus leading, grouping per `CURRENCY_CONFIG` locale), `gainLossClass` / `gainLossToneClass`, `formatSignedCurrency` (the same string as `formatCurrency` — kept as the call-site marker for a P&L figure), `formatSignedPercent`, `formatCompactCurrency`, `isStale`, `getStalenessLevel` (returns `StalenessLevel` from `lib/constants/prices`). Quantities format through `DISPLAY_LOCALE` (`lib/constants/app`). Plus `priceMapsEqual` / `ratesEqual` — value-equality used by the poll's no-op-`setState` guard (see polling loop). |
