...
- **Page**: a single React route, `src/pages/SettingsPage.tsx` — a thin shell of
  shadcn `Tabs` mounting **Assets**, **Platforms**, and **Snapshots** (its title
  block is the shared `PageHeading` — `src/components/common/PageHeading.tsx`,
  `hidden md:block`; on a phone the app header is the only page title), plus a
...
| `src/contexts/DisplayContext.tsx` | Display currency + obfuscation. `localStorage` keys **`"portfolio-display-currency"`** (`"USD"\|"TRY"`, default USD) and **`"portfolio-obfuscated"`**. Exposes `useDisplayCurrency()` → `{ currency, toggleCurrency, obfuscated, toggleObfuscated }`. `currency` is typed `DisplayCurrency` (`lib/constants/currencies.ts`) — deliberately narrower than `FiatCurrency`, which includes the EUR assets are priced in but totals are never shown in; every prop that carries "the currency the user is looking at" takes that type. |
