...
- `src/components/layout/Sidebar.tsx` — desktop-only (`hidden md:flex`) left nav; owns the nav model: `primaryNavItems` (Dashboard, Portfolio, Transactions), `secondaryNavItems` (flag-gated Performance, Retirement, Budget, Campaigns, Settings), `navItems` (their concat, rendered by the sidebar), and `moreNavItem` (`/more`, "More", `LayoutGrid`) — all exported for `MobileNav`, `MorePage`, and `Header`; footer row renders `<BuildBadge/>`.
...
- `src/components/layout/MobileNav.tsx` — fixed bottom nav (`md:hidden`): 4 tabs — `primaryNavItems` + the More tab. More is highlighted when `pathname` is `/more` or starts with any `secondaryNavItems` path (computed via `useLocation`, not `NavLink`'s `isActive`).
- `src/pages/MorePage.tsx` — the `/more` hub: renders `secondaryNavItems` as a bordered, divided list of `NavLink` rows (icon + label + chevron), `max-w-md` centered.
- `src/components/layout/Header.tsx` — top bar: mobile page title (`pageTitles` derived from `navItems` + `moreNavItem`), hide/show-values toggle, `ThemeToggle`, `CurrencyToggle`, `PriceRefreshButton`, `UserMenu`.
