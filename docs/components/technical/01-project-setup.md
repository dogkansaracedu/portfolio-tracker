...
- `src/components/common/HintPopover.tsx` — the app's ONE explainer affordance: a `Popover` whose trigger sets `openOnHover` (hover with a mouse, tap with a finger, one code path), a real `<button>` padded to a 40px target below `sm` (`-m-2 p-2 sm:m-0 sm:p-0`). Replaces every hover-only `Tooltip`/`title` explainer — retirement's `Hint`/`HintLabel`, the hero's MWR/yr chip and measure switch, the Portfolio bar's MWR chip, Asset Detail's `MwrLabel`. `SegmentedControl` deliberately has no `hint`/`title` prop any more.
