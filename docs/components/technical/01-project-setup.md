...
- `src/lib/constants/navigation.ts` — the nav model, shared by all four shells: `primaryNavItems` (Dashboard, Portfolio, Transactions), `secondaryNavItems` (flag-gated Performance, Retirement, Budget, Campaigns, Settings), `navItems` (their concat, rendered by the sidebar), `moreNavItem` (`/more`, "More", `Ellipsis`), and the shared route predicates `matchesPath` (exact or sub-path) / `isNavItemActive` (adds `/assets/*` → Portfolio). It is a plain module, not a component file, so `Sidebar` can stay a component-only export (Vite fast refresh).
- `src/components/layout/Sidebar.tsx` — desktop-only (`hidden md:flex`) left nav; renders `navItems` as plain `Link`s with active state from `isNavItemActive` + `aria-current`; footer row renders `<BuildBadge/>`.
...
- `src/components/layout/MobileNav.tsx` — fixed bottom nav (`md:hidden`): 4 tabs — `primaryNavItems` + the More tab, rendered as plain `Link`s with active state from the shared `navigation` predicates and explicit `aria-current="page"`. More is highlighted on `/more` and on any `secondaryNavItems` path; `/assets/*` highlights the Portfolio tab (asset detail is its drill-down) — same rule as the desktop sidebar.
- `src/pages/MorePage.tsx` — the `/more` hub: the shared `PageHeading` ("More", no subtitle) + `secondaryNavItems` as a bordered, divided list of `Link` rows (icon + label + chevron, `active:bg-accent` press feedback), `max-w-md`.
...
- `src/hooks/useMediaQuery.ts` — `useMediaQuery(query)` + `MEDIA_QUERY` (Tailwind's own `sm`/`md`/`lg`/`xl` widths). Only for what CSS cannot reach: chart libraries size their plot area in JS, so an axis hidden by a Tailwind variant would still reserve its width.
- `src/components/common/HintPopover.tsx` — the app's ONE explainer affordance: a `Popover` whose trigger sets `openOnHover` (hover with a mouse, tap with a finger, one code path), a real `<button>`. With no children it renders the info glyph inside a 40px box below `sm` (`max-sm:-m-3 max-sm:size-10` — the negative margin keeps it to the glyph's room in the line it sits on); a caller-supplied chip is already wide enough to hit and stays inline. Replaces every hover-only `Tooltip`/`title` explainer — retirement's `Hint`/`HintLabel`, the hero's MWR/yr chip and measure switch, the Portfolio bar's MWR chip, Asset Detail's `MwrLabel`. `SegmentedControl` deliberately has no `hint`/`title` prop any more.
- `src/components/common/PageHeading.tsx` — every page's title block: `hidden md:block` over an `h1` + optional subtitle. The breakpoint is the app-wide rule (the header already names the screen on a phone), not a per-page choice, so Portfolio, Transactions, Retirement, Budget, Settings, Campaigns and More all render this one component.
- `src/components/common/SeriesDot.tsx` — a chart series' colour swatch, inline in the chip or toggle that names the series (this app puts its legends on existing chips, never in a legend row of their own). Shared by the dashboard hero and Asset Detail's history chart.
- `src/lib/constants/charts.ts` — `CHART_TOOLTIP_CONTENT_STYLE`, the one Recharts tooltip skin: Recharts paints its tooltip with inline styles, so it cannot inherit the theme through a class. Used by the hero and all three retirement charts; a chart needing more (`PlanChart`'s width cap) spreads it first.
