...
- `src/types/database.ts` — hand-written row interfaces mirroring the schema (`Platform`, `Asset`, `Holding`, `Transaction`, `PriceCache`, `Snapshot`, `ExchangeRate`, `BenchmarkPrice`, `RetirementScenario`), the `TransactionType` union, the `SnapshotBreakdown` shape, and `*Insert` / `*Update` helper types (numeric-as-string on writes). Kept in sync with migrations **by hand** — there is no generated-types step in the loop.
