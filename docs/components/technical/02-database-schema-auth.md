...
| `20260530000311_asset_yahoo_retarget.sql` | Retargets crypto + tokenized gold to Yahoo `*-USD` symbols, renames stablecoins (display only), re-keys `price_cache`. |
| `20260531000000_asset_icon_url_column.sql` | Adds `assets.icon_url` (optional manual logo override). Additive. |
| `20260601000000_stablecoin_yahoo_retarget.sql` | Retargets USDT/USDC to Yahoo, re-keys `price_cache`, and re-defines `seed_user_data` to the Yahoo-priced model. |
...
| `20260809175909_create_manual_prices.sql` | Adds global `manual_prices` table (hand-entered historical closes, PK `price_id`+`price_date`, TRY default) read by `backfill-snapshots` to fill dates Yahoo/TEFAS lack (fresh BIST IPOs). Shared-read RLS (`SELECT` to authenticated), no write policy — rows entered via SQL/service role. |
| `20260809192006_add_tax_transaction_type.sql` | Adds `tax` to the `transaction_type` enum — taxes charged to a cash account (e.g. Midas' monthly fund stopaj). Subtracts from the fiat balance; neutral to net invested and never an external flow (the charge surfaces as P&L loss). |
| `20260815120000_retirement_scenarios.sql` | Adds per-user `retirement_scenarios` table (`name`, `is_default`, `inputs jsonb` mirroring `RetirementScenarioInputs`) + index `idx_retirement_scenarios_user_created` + partial unique index `idx_retirement_scenarios_one_default_per_user` on `(user_id) WHERE is_default` (the "exactly one default" invariant, DB-enforced) + four `auth.uid() = user_id` RLS policies. Backs Component 13's persistence contract. |
| `20260819120000_interest_positions.sql` | Adds per-user `interest_positions` table (`asset_id`/`platform_id` FKs into the user's own catalog, `quantity`, nullable `apr`/`apr_kind`, `label`, `started_at`, nullable `expires_at` = flexible, `campaign_id` FK `ON DELETE SET NULL`, `note`, `is_closed`) + index `idx_interest_positions_user_open` on `(user_id, is_closed)` + four `auth.uid() = user_id` RLS policies. Backs Component 16. Informational only: no trigger, no ledger effect — rows never touch `holdings`, `transactions` or any P&L figure. |

**Types & client:**

- `src/types/database.ts` — hand-written row interfaces mirroring the schema (`Platform`, `Asset`, `Holding`, `Transaction`, `PriceCache`, `Snapshot`, `ExchangeRate`, `BenchmarkPrice`, `RetirementScenario`, `InterestPosition`), the `TransactionType` union, the `SnapshotBreakdown` shape, and `*Insert` / `*Update` helper types (numeric-as-string on writes). Kept in sync with migrations **by hand** — there is no generated-types step in the loop.
...
**RLS / isolation.** Each per-user table (`platforms`, `holdings`, `transactions`, `snapshots`, `intraday_snapshots`, `retirement_scenarios`, `interest_positions`) has four `auth.uid() = user_id` policies (`select`/`insert`/`update`/`delete`). `assets` follows the **shared-read** pattern instead: a single `SELECT … TO authenticated USING (true)` so every authenticated user reads the whole catalog, plus INSERT/UPDATE/DELETE policies gated to the hardcoded admin uuid (`auth.uid() = '201091b3-6381-48f2-860b-4947fac09c69'`) — see migration `20260610000000_global_asset_catalog.sql`. `price_cache`, `exchange_rates`, `benchmark_prices`, `manual_prices` have a single `SELECT … TO authenticated USING (true)` and **no** write policy — service-role writes (edge functions) bypass RLS. `signup_allowlist` has RLS enabled and **zero** policies, so only the service role and `SECURITY DEFINER` functions can touch it.
...
- `init.sql` ships an **empty** `signup_allowlist`, so a DB created from the baseline starts with nobody allowed through except rows an operator inserts. The live DB carries its own accumulated rows.
...
**Seeding.** `public.seed_user_data(p_user_id uuid)` (`SECURITY DEFINER`, guarded by `IF p_user_id IS DISTINCT FROM auth.uid() THEN RAISE`) inserts the defaults. **The authoritative body lives in `20260610000000_global_asset_catalog.sql`** (the last `CREATE OR REPLACE` in the migration chain). It seeds **8 platforms only** (IBKR, Midas, Midas Kripto, Paribu, OKX, Binance, Enpara, Fiziksel) and **no assets** — assets are the single global catalog every user already shares, so they are not re-seeded per user. Execute is revoked from `PUBLIC`, granted to `authenticated`. Called from `AuthContext.signUp` post-signup.
...
- **New users get 0 assets.** `seed_user_data` seeds **8 platforms and no assets** — every user reads the shared global asset catalog instead.
- **Three `seed_user_data` definitions exist across the migration chain** (`init.sql`, then `20260601000000_…`, then `20260610000000_…`, each `CREATE OR REPLACE`ing the last). Last-applied wins: `20260610000000_…` is the source of truth, and it is the only one to edit.
- **Session identity guard (both paths).** `AuthContext` routes `getSession()` *and* `onAuthStateChange` through one `applySession` that bails out of `setUser`/`setSession` when `access_token` / `user.id` are unchanged. Two hazards need this: (1) on mount `getSession()` and `onAuthStateChange`'s `INITIAL_SESSION` race — whichever lands second carries the same id but a fresh object reference, and an unguarded set flips `user` a second time, firing every `[user]` effect twice (two identical `fetchTransactionsForAllAssets` calls, doubled snapshot/holdings loads); (2) `TOKEN_REFRESHED` on tab-focus return swaps the reference the same way. Keep both paths guarded.
