...
- **App shell.** Authenticated shell screens are wrapped by a persistent layout:
  ...
  - The active screen is visually indicated in whichever navigation is showing. On mobile, while the user is on any secondary screen (or the hub itself), the More tab is the highlighted one; a drill-down screen (e.g. an asset's detail) keeps its parent screen's tab highlighted and shows a matching header title.
